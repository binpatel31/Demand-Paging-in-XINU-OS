...
disable(ps);
  /* requests a new mapping of npages with ID map_id */
    ...
    int pagesValue        = bsm_tab[bs_id].bs_npages[currpid];
    if (bs_id >= 16 || bs_id < SETZERO || npages > 128 ||
    ...
    if (checkPrivateValue == SETZERO && checkStatus == 1) {
      /* code */
      restore(ps);
      pagesValue = bsm_tab[bs_id].bs_npages[currpid];
      return pagesValue;
    }

    pagesValue = npages;

    bsm_tab[bs_id].bs_npages[currpid] = pagesValue;
