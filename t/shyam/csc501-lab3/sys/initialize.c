...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	for(i=0; i<4; i++){			/* to map 16MB XINU memory we require 4page tables, populate first 4 page tables  */
		get_frm(&frame_number);
		pd_pointer = ((FRAME0+i)*NBPG);
		...
		int k;
		for(k=0; k< 1024 ; k++){			/* insert 1024 entries for page tables  */
